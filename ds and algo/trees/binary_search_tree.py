...
'''Define insert
Let's assume that duplicates are overriden
 by the new node that is to be inserted. Other options are to
  keep a counter of duplicate nodes, 
...
'''
Search
Define a search function that takes a value, and returns true if a 
node containing that value exists in the tree, otherwise false.
'''
...
print(f"""
search for 8: {tree1.search(8)}
search for 2: {tree1.search(2)}
""")
print(tree)